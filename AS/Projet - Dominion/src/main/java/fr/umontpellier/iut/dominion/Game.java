...
public class Game {
    ...
    public List<Player> otherPlayers(Player p) {
        ...
        for (int i = list.size() - 1; i > list.indexOf(p); i--)
        {
    ...
    public ListOfCards availableSupplyCards() {
        ListOfCards cartesDispo = new ListOfCards();
        for (ListOfCards liste :
                supplyStacks) {
            if (!liste.isEmpty())
                cartesDispo.add(liste.get(0));
        }
        return cartesDispo;
    }
    ...
    public Card getFromSupply(String cardName) {
        for (ListOfCards liste :
                supplyStacks) {
            Card carteCherchee = liste.getCard(cardName);
            if (carteCherchee != null)
    ...
    public Card removeFromSupply(String cardName) {
        for (ListOfCards liste :
                supplyStacks) {
            Card carteRetiree = liste.remove(cardName);
    ...
    /**
     * On suppose que la carte a déjà été enlevé de son origine
     *
     */
    ...
    public int emptyStacks() {
        int nbEmptyStacks = 0;
        for (ListOfCards list :
                supplyStacks) {
            if (list.isEmpty())
                nbEmptyStacks++;
        }
        return nbEmptyStacks;
    }
