...
public class Player {
    ...
    private int numberOfMerchantsThroned;
    ...
    public int getNumberOfMerchantsThroned() {
        return numberOfMerchantsThroned;
    }
    ...
    public ListOfCards getAllCards() {
        ...
        ListOfCards res = new ListOfCards();
        res.addAll(hand);
        res.addAll(discard);
        res.addAll(draw);
        res.addAll(inPlay);
    ...
    public int getVictoryPoints() {
        int sumPoints = 0;
        ListOfCards allCards = getAllCards();
        for (Card c :
                allCards) {
            sumPoints += c.getVictoryValue(this);
        }
        return sumPoints;
    }
    ...
    //METHODE AJOUTE JULIEN
    public void incrementMerchantsThroned(int n) {
        numberOfMerchantsThroned += n;
    }
    ...
    public Card drawToHand() {
        Card cartePiochee = drawCard();
        if (cartePiochee != null) {
            addToHand(cartePiochee);
            return cartePiochee;
        } else {
            return null;
        }
    ...
    public Card gainFromSupply(String cardName) {
        Card carteGagnee = game.removeFromSupply(cardName);
        gain(carteGagnee);
        return carteGagnee;
    }
    ...
    public Card buyCard(String cardName) {
        Card carteAchetee = game.getFromSupply(cardName);
        if (carteAchetee != null && carteAchetee.getCost() <= money && numberOfBuys > 0) {
            gainFromSupply(cardName);
            incrementMoney(-(carteAchetee.getCost()));
            incrementBuys(-1);
            return carteAchetee;
        }
        return null;
    ...
    public void endTurn() {
        ...
        while (!inPlay.isEmpty())
            discardCard(inPlay.remove(0));
        for (int i = 0; i < 5; i++)
            drawToHand();
    }
    ...
    //ganer une carte dans les main
    public Card gainInHand (String cardName) {
        Card carteGagne = game.removeFromSupply(cardName);
        addToHand(carteGagne);
        return carteGagne;
    }

    //METHODE AJOUTE JADE
    public Card removeFromHand(String nomCarte){
        return hand.remove(nomCarte);
    }
    ...
    //METHODE AJOUTE RODIEN
    public Moat haveMoat () {
        for (Card c :
                hand) {
    ...
    //METHODE AJOUTE RODIEN
    public boolean reactMoat() {
        Moat moat = haveMoat();
        if (moat != null)
            return moat.choixUtiliser(this);
        return false;
    }
